...
public class Inventory {
	...
	public void commitOrder(ArrayList<Item> cart) {
        for(int i = 0; i < cart.size(); i++){
            int index = this.itemList.indexOf(cart.get(i));
            int cartIndex = 0;
                       
            if(cart.get(i).getQuantity() < this.itemList.get(index).getQuantity()) {
                this.itemList.get(index).setQuantity(this.itemList.get(index).getQuantity() - cart.get(i).getQuantity());
